...
void suplier::function_1(suplier* m) { //Добавить новый элемент
	...
	gets_s(buf);
	delete[] t[n].firma;
	t[n].firma = new char[strlen(buf) + 1];
	t[n].set_firma(buf);
	...
	gets_s(buf);
	delete[] t[n].adres;
	t[n].adres = new char[strlen(buf) + 1];
	t[n].set_adres(buf);
	...
	//день
	cout << "День: ";
	t[n].date.day = scan_int();

	//месяц
	cout << "Месяц(слово): ";
	scan_month(buf);
	delete[] t[n].date.month;
	t[n].date.month = new char[strlen(buf) + 1];
	t[n].date.set_month(buf);

	//год
	cout << "Год: ";
	t[n].date.year = scan_int();
	...
	*typ = scan_int();
	while (*typ != 1 && *typ != 2) {
		*typ = scan_int();
		...
		printf("Введено некорректное зачение, попробуйте еще раз: ");
		*typ = scan_int();
	}
	print_tabl();
	for (int i = 0; i < n; i++) {
...
void suplier::function_6(suplier* m) {
	...
	print_tabl();
	for (int i = 0; i < n; i++) {
		int month = month_in_digit(date_now);
		int month_now = month_in_digit(&m[i]);
